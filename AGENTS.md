...
- При правке файла — не удалять существующие why/sentinel-комментарии без доказательства, что они ложные или не критичны.
...
- Любые локальные тесты, сборки и lint/style-прогоны запрещены без исключений. Полная валидация выполняется только через CI или по отдельному явному разрешению пользователя.
## Подход к проблемам
...
## Workflow
- Последнее действие перед Codex PR: удалить все ignored/untracked outputs, включая `.gradle/`, `build/`, `buildSrc/{.gradle,build}/`, `.memory/.venv/`; после очистки ничего не запускать — web update сканирует workspace binaries вне Git diff.
- `.memory/` не коммитить и не пушить отдельным самостоятельным шагом. Если `.memory/` dirty, включать её в ближайший рабочий коммит вместе с кодом/правилами; отдельный memory-коммит только по прямой команде пользователя.
- main не трогать без явной команды.
- Merge dev→main: БЕЗ сквоша (`git merge --ff` или `--no-ff`), история 1:1 с dev.
- Релизы: каждый merge dev→main запускает release.yml и публикует APK; тег `v*.*.*` остаётся ручным override версии.
## Билд
