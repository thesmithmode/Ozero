...
- Локальные Gradle test/build/lint проверки запрещены, кроме `ktlint + detekt`: перед push запускать локально тот же style-gate, что в CI. Полная валидация — через CI.
## Подход к проблемам
