...
class EngineRuntimeConfigRestartObserverTest {
    ...
    @Test
    fun `observeFlow keeps old baseline when restart reports failure`() = runTest(dispatcher) {
        val changes = MutableStateFlow<Any?>("initial")
        val state = MutableStateFlow<TunnelState>(TunnelState.Connected(EngineId.WARP, 0))
        val restarts = mutableListOf<String>()
        ...
        assertEquals(listOf("warp changed", "warp changed"), restarts)
    }
    ...
    fun `runtime observer stays generic while composition root wires engine providers`() {
        ...
        assertTrue(coordinator.contains("ACTION_RESTART_RUNTIME_CONFIG"))
        assertTrue(coordinator.contains("context.startService("))
        ...
        assertTrue(observer.contains("private data class ObserveState"))
