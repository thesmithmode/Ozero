...
class OzeroQuickTileSentinelTest {
    ...
    fun `syncTileState маппит Connected Connecting Probing в STATE_ACTIVE`() {
        val source = locateTile().readText()
        assertTrue(
            source.contains("TunnelState.Connected") && source.contains("TunnelState.Connecting") &&
                source.contains("TunnelState.Probing") && source.contains("Tile.STATE_ACTIVE"),
    ...
    fun `syncTileState маппит Idle Failed Disconnecting в STATE_INACTIVE`() {
        val source = locateTile().readText()
        assertTrue(
            source.contains("TunnelState.Idle") && source.contains("TunnelState.Failed") &&
                source.contains("TunnelState.Disconnecting") && source.contains("Tile.STATE_INACTIVE"),
