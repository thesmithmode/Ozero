...
class CoreModelCoverageTest {
    ...
    fun `sealed result models expose payloads and equality`() {
        ...
        val probeFailure = ProbeResult.Failure("timeout", code = ProbeResult.Failure.Code.ROUTED_PROBE_FAILED)
        assertEquals("timeout", probeFailure.reason)
        assertEquals(ProbeResult.Failure.Code.ROUTED_PROBE_FAILED, probeFailure.code)
