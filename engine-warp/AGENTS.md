...
`EngineWarp.resolveEndpointHost()` резолвит hostname → IP до VPN establish,
иначе DNS запрос маршрутизируется через туннель который ещё не поднят → loop.

В тестах: использовать IP в peerEndpoint, не hostname — иначе тест делает реальный DNS запрос.
