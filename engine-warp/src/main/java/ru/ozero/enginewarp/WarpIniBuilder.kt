...
object WarpIniBuilder {
    ...
    private fun StringBuilder.appendAwgIfPresent(p: AwgParams) {
        ...
        if (p.payloadHexI3 != null) appendLine("I3 = <b 0x${p.payloadHexI3}>")
        else if (p.specialJunk3 != 0) appendLine("I3 = ${p.specialJunk3}")
        if (p.payloadHexI4 != null) appendLine("I4 = <b 0x${p.payloadHexI4}>")
        else if (p.specialJunk4 != 0) appendLine("I4 = ${p.specialJunk4}")
