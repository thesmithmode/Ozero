...
object ClashYamlParser {
    ...
    fun parse(yaml: String): ClashParseResult {
        ...
        val commonData = collectAnchors(lines).values.firstOrNull { "private-key" in it }
        ...
        return buildResult(endpoints, commonData)
    }
    ...
    private fun buildResult(endpoints: List<String>, data: Map<String, String>): ClashParseResult {
        val awgBlock = data["_awg"] ?: throw ParseException("amnezia-wg-option не найден")
        return ClashParseResult(
            endpoints = endpoints,
            privateKey = data["private-key"] ?: throw ParseException("private-key отсутствует"),
            peerPublicKey = data["public-key"] ?: throw ParseException("public-key отсутствует"),
            interfaceAddressV4 = data["ip"]?.withCidr("/32") ?: "",
            interfaceAddressV6 = data["ipv6"]?.withCidr("/128") ?: "",
            dnsServers = data["dns"]?.let(::parseDnsList) ?: WarpConfig.DEFAULT_DNS,
    ...
    private fun collectAnchors(lines: List<String>): Map<String, Map<String, String>> {
        ...
        while (i < lines.size) {
            ...
            if (line.length == trimmed.length) {
                ...
                if (m != null) {
                    val (data, nextI) = parseAnchorBlock(lines, i + 1)
                    anchors[m.groupValues[1]] = data
                    i = nextI
                    continue
                }
            }
    ...
    private fun parseAnchorBlock(lines: List<String>, startIdx: Int): Pair<Map<String, String>, Int> {
        ...
        while (i < lines.size) {
            val inner = lines[i]
            if (inner.isBlank() || inner.trimStart().startsWith('#')) {
                i++
                continue
            }
            ...
            when {
                innerTrimmed.startsWith("amnezia-wg-option:") -> {
                    inAwg = true
                    i++
                }
                inAwg -> {
                    awgLines.add(innerTrimmed)
                    i++
                }
                else -> {
                    inAwg = false
    ...
    private fun collectEndpoints(lines: List<String>): List<String> {
        ...
        while (i < lines.size) {
            ...
            if (trimmed.startsWith("- ")) {
                val (endpoint, nextI) = parseProxyEndpoint(lines, i + 1, indent)
    ...
    private fun parseProxyEndpoint(
        lines: List<String>,
        startIdx: Int,
        outerIndent: Int,
    ): Pair<String?, Int> {
        ...
        return Pair(if (server != null && port != null) "$server:$port" else null, i)
    }
    ...
    private fun String.unquote(): String {
        val isDoubleQuoted = length >= 2 && startsWith('"') && endsWith('"')
        val isSingleQuoted = length >= 2 && startsWith('\'') && endsWith('\'')
        return if (isDoubleQuoted || isSingleQuoted) substring(1, length - 1) else this
    }
}
