...
- Sentinels must guard the real boundary, including runtime-relevant provider fingerprints, not only the absence of engine imports in `MainActivity`.
...
The boundary also owns precision. WARP fingerprints need runtime-affecting config fields, FPTN must ignore `selectedServerName` while `autoSelect=true`, and sing-box must avoid treating internal startup writes as user runtime edits. Provider-local logic is the only layer that can make those distinctions without turning `app/vpn` into a switchboard of engine details.
...
- [[daily/2026-05-31]]: Sessions 22:09 and 23:08 record that runtime fingerprints must include full WARP runtime config while excluding FPTN fields ignored by auto-select mode.
