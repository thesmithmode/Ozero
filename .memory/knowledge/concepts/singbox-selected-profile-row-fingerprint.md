...
- Manual mode should fingerprint the row selected by `selectedProfileId` or synchronize the cached bean from that row before fingerprinting.
## Details
...
The daily log specifically notes that subscription refresh or profile edit can update `ProxyProfileDao` without changing `SELECTED_PROFILE_KEY` or `BEAN_KEY`. In that case, a DataStore-only fingerprint is stale by construction. The provider must either read the active row by id during fingerprint generation or keep the cached bean synchronized with DAO updates so runtime restart observes the real selected profile payload.
## Related Concepts
