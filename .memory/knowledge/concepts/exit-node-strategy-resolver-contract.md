...
The 2026-05-29 implementation moved exit-node policy out of `MainViewModel`. Engines without a safe strategy should default to `Unavailable`; URnetwork remains location-only, WARP remains a provider label until a safe route is proven, and sing-box uses its routed SOCKS probe.
## Related Concepts
...
- [[connections/multi-engine-lifecycle-exitnode-regression-loop]] - Shows why exit-node proof was handled after engine lifecycle fixes.
## Sources
...
- [[daily/2026-05-29]]: records that `MainViewModel` direct fallback was identified as unsafe and replaced by the resolver contract.
