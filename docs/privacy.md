...
| Crash-логи | `filesDir/crashes/<ts>.txt` | диагностика, ручной экспорт по выбору пользователя |
...
| `REQUEST_INSTALL_PACKAGES` | self-update через PackageInstaller |
| `REQUEST_IGNORE_BATTERY_OPTIMIZATIONS` (опц.) | один раз предложить whitelist от Doze |
