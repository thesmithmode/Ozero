...
## E13.1 — Регрессионные сценарии (manual)

Запустить APK на реальном устройстве (не эмулятор). Mobile + Home broadband
для нескольких сетей (минимум 3 разных ISP), сценарий — стандартный набор
популярных HTTPS-сервисов из списка `DiagnosticsTester`.

**Критерий прохода:** автовыбор движка работает на проверочных URL
без ручного вмешательства.

**Ручной fallback test:** в каждой сети по очереди форсировать engine
через Settings → manual engine — все engines должны хотя бы один
раз отработать.
