# MasterDNS — поднять свой сервер

DNS-туннельный движок Ozero (`MasterDNS`) — обёртка над [MasterDnsVPN](https://github.com/masterking32/MasterDnsVPN). Туннелирует трафик через DNS-запросы к собственному authoritative-серверу. Подходит как экстренный fallback при недоступности стандартных VPN-протоколов. Скорость ~1–3 Мбит/с.

Шифр для движка: AES-256-GCM или ChaCha20-Poly1305. Свой сервер обязателен — публичных нет.

## Способ A — авторазвёртывание из приложения (рекомендуется, 2 минуты)

Ozero может развернуть сервер за вас по SSH:

1. Открыть Ozero → Настройки → MasterDNS.
2. В разделе «Автодеплой сервера» ввести IP/хост VPS, SSH-порт (22), логин (`root`), пароль.
3. Нажать «Развернуть сервер».

Что приложение делает на сервере:

- проверяет, что у пользователя sudo без пароля и порт 53/UDP свободен;
- ставит Docker (apt/dnf/yum/zypper/pacman автоматически выбирается);
- собирает Docker-образ `masterdns-ozero` из upstream-скрипта;
- запускает контейнер с привязкой 53/udp, генерирует encryption key;
- открывает 53/udp в firewall (ufw/firewalld/iptables);
- читает encryption key и автоматически заполняет client_config.toml, серверный IP и список resolvers в приложении.

Прогресс и лог шагов отображаются в UI прямо во время установки. Те же шаги логируются в `PersistentLoggers` (Настройки → Логи).

Требования к VPS: ≥512 МБ RAM, 1 vCPU, ≥1 ГБ свободного диска, доступ по SSH с правами sudo без пароля, открытый порт 53/UDP во внешний интернет.
