...
class TunBuilderConfigurator(
    ...
    // SENTINEL [feedback_split_tunnel_exclude_self]: custom-TUN движки задают excludeSelf через TunSpec.
    // IP-probe должен идти через EnginePlugin.ipProbeRoute(), не через split tunnel.
    fun apply(
