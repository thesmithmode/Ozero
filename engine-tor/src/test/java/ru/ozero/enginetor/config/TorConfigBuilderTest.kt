...
class TorConfigBuilderTest {
    ...
    fun rendersBaseFields() {
        val cfg = builder.build(emptyList())
        assertTrue(cfg.contains("SocksPort 9050"))
        assertTrue(cfg.contains("ControlPort 9051"))
        assertTrue(cfg.contains("DataDirectory /data/local/tmp/tor"))
    }
    ...
    fun usesBridgesWhenProvided() {
        val cfg = builder.build(
            listOf(obfs4()),
            TorBuildOptions(ptBinaries = mapOf("obfs4" to "/lib/obfs4proxy")),
        )
    ...
    fun rendersMultipleTransports() {
        val cfg = builder.build(
            ...
            TorBuildOptions(
                ptBinaries = mapOf("obfs4" to "/lib/obfs4proxy", "snowflake" to "/lib/snowflake"),
            ),
        )
        assertTrue(cfg.contains("ClientTransportPlugin obfs4 exec /lib/obfs4proxy"))
    ...
    fun rejectsMissingPtBinaryForUsedTransport() {
        val ex = runCatching {
            builder.build(listOf(obfs4()), TorBuildOptions(ptBinaries = emptyMap()))
        }.exceptionOrNull()
    ...
    fun rejectsSocksEqControlPort() {
        val ex = runCatching {
            builder.build(emptyList(), TorBuildOptions(socksPort = 9050, controlPort = 9050))
        }.exceptionOrNull()
        assertTrue(ex is IllegalArgumentException)
    }
    ...
    fun deterministicOutput() {
        val a = builder.build(listOf(obfs4()), TorBuildOptions(ptBinaries = mapOf("obfs4" to "/p")))
        val b = builder.build(listOf(obfs4()), TorBuildOptions(ptBinaries = mapOf("obfs4" to "/p")))
        assertEquals(a, b)
    }
}
