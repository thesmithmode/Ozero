...
class DeviceWalletJwtSentinelTest {
    ...
    @Test
    fun `acquireDeviceWalletJwt подписывает fresh scoped сообщение для конкретного pubkey`() {
        assertTrue(
            authSource.contains("WALLET_MESSAGE_PREFIX") &&
                authSource.contains("\"ozero-auth-v1:\"") &&
                authSource.contains("|aud=urnetwork") &&
                authSource.contains("|op=") &&
                authSource.contains("|iat_ms=") &&
                authSource.contains("|nonce="),
            "walletAuth message обязан включать prefix, audience, operation, timestamp и nonce.",
        )
    }
    ...
    fun `authLogin вызывается перед networkCreate - returning user получает существующий byJwt`() {
        val loginIdx = authSource.indexOf("authLoginWithWallet(api, loginAuth)")
        val createIdx = authSource.indexOf("networkCreateWithWallet(api, createAuth, networkName)")
